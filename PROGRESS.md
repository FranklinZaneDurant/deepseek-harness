...
- 当前已验证状态：工作区未提交（基于 `2f2308076b`）。验证门 `desktop-bundled-plugin-provisioning` 已置 passing：
...
- 下一步：
  ...
  2. 决定是否把 `@huiliyi37/dsh-office` 的 pin 版本与 peer 兼容问题（`^0.1.0-rc.5` 不匹配 `0.1.5-rc.2`）作为发布门槛固化进打包脚本。
